...
import { deleteHorse, getHorseWithListFallback, updateHorse } from '@/lib/api/horses-service';
...
export async function PUT(request: NextRequest, { params }: HorseRouteProps) {
  const { id } = await params;
  const locale = (request.nextUrl.searchParams.get('locale') === 'en' ? 'en' : 'ar') as LocaleCode;

  try {
    const formData = await request.formData();
    const result = await updateHorse(id, formData);
    return NextResponse.json(result ?? { succeeded: true, statusCode: 200 });
  } catch (error) {
    const status = error instanceof ApiError ? error.status : 500;
    const message = error instanceof Error ? error.message : null;
