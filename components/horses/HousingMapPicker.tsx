...
const unitLabel = (unit: HousingUnitDto, locale: 'ar' | 'en') => {
  const value = (locale === 'ar' ? unit.nameAr : unit.nameEn) || unit.code;
  return locale === 'ar'
      ...
      <div className="flex flex-col gap-3 border-b border-[#ece2da] bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-[#8b796e]">
            {locale === 'ar'
              ? 'مرر فوق أي مكان لإظهار رقمه وتفاصيله، ثم اختره.'
              : 'Hover any slot to show its number and details, then select it.'}
          </p>
        </div>
            ...
            {units.map((unit) => {
              ...
              return (
                <g
                  ...
                  aria-label={`${unitLabel(unit, locale)}, ${unit.horses.length}/${unit.capacity}`}
                  onKeyDown={(event) => {
  ...
  return (
    <>
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="font-bold text-[#2f2118]">{unitLabel(unit, locale)}</div>
      ...
      {unit.type !== 'barn' && (
        ...
        >
          {locale === 'ar' ? 'اختيار هذا المكان' : 'Select this slot'}
        </button>
      )}
      ...
      {unit.horses.length > 0 ? (
        ...
        <div className="mt-3 rounded-xl bg-[#eff6ed] px-3 py-2 text-xs font-semibold text-[#4e7653]">
          {locale === 'ar' ? 'هذا المكان متاح للتعيين.' : 'This slot is available for assignment.'}
        </div>
      )}
...
function UnitHint({ unit, locale }: { unit: HousingUnitDto; locale: 'ar' | 'en' }) {
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-base font-bold">{unitLabel(unit, locale)}</div>
          <div className="mt-1 text-xs text-white/75">
            {unit.type === 'barn'
              ? locale === 'ar'
                ? 'اضغط لعرض أماكن العنبر'
                : 'Click to view shared slots'
              : locale === 'ar'
                ? 'اضغط لاختيار المكان'
