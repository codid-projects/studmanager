  ...
  return <div className="fixed inset-0 z-[110] grid place-items-center bg-[#24150d]/55 p-4 backdrop-blur-[2px]" onMouseDown={event => event.target === event.currentTarget && !saving && onClose()} dir={ar ? "rtl" : "ltr"}>
    <section className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-[0_28px_90px_rgba(30,17,10,.35)]">
      ...
      {isSold && !editing ? <div className="space-y-3 p-5">
        ...
        <label className="block text-sm font-semibold text-[#4a382e]">{ar ? "تم البيع إلى" : "Sold to"}<input maxLength={250} value={buyer} onChange={event => setBuyer(event.target.value)} placeholder={ar ? "اسم المشتري (اختياري)" : "Buyer name (optional)"} className="mt-2 h-12 w-full rounded-xl border border-[#d9cec6] px-4 outline-none focus:border-[#4b2d1c]" /></label>
        <label className="block text-sm font-semibold text-[#4a382e]">{ar ? "قيمة البيع" : "Sale price"}<input maxLength={100} value={price} onChange={event => setPrice(event.target.value)} placeholder={ar ? "مثال: 250,000 جنيه (اختياري)" : "Example: EGP 250,000 (optional)"} className="mt-2 h-12 w-full rounded-xl border border-[#d9cec6] px-4 outline-none focus:border-[#4b2d1c]" /></label>
        {error && <p className="rounded-lg bg-red-50 p-3 text-sm text-red-600">{error}</p>}
